...
// 🔥 3. 引入生成的车辆数据
// 如果报错 "Module not found", 请确保 src/carsData.js 存在 (需运行 node generateData.js)
...
const getCarImage = (folderName, imageCount, type = 'cover') => {
    // 安全检查：如果 folderName 是空的，直接返回默认图，防止报错
    if (!folderName) {
...
const InventoryPage = ({ cars, category }) => {
    ...
    let filteredCars = safeCars;
    let pageTitle = "Current Inventory";

    if (category === 'toyota') {
        // 🔥 修复：这里加了安全判断，防止 folderName 为空时报错
        filteredCars = safeCars.filter(car => {
            const title = car.title || "";
            const folder = car.folderName || "";
            // 统一转小写再匹配，防止大小写对不上
            const searchString = `${title} ${folder}`.toLowerCase();
            return searchString.includes("alphard") || searchString.includes("vellfire");
        });
        ...
        <div className="container mx-auto px-4 py-12">
            ...
            {filteredCars.length === 0 ? (
                <div className="text-center py-20 text-gray-500">
                    <p className="text-xl mb-2">No vehicles found in this collection.</p>
                    <p className="text-sm">Please check back later or view our full inventory.</p>
                </div>
            ) : (
                <>
    ...
    if (!car) {
        return (
            <div className="p-20 text-center">
                ...
                <button onClick={() => navigate('/inventory')} className="text-red-600 font-bold hover:underline">Return to Inventory</button>
            </div>
        );
    }
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                <div className="lg:col-span-8 space-y-4">
                    ...
                    <div className="mt-8 pt-8 border-t border-gray-100">
                        ...
                        <p className="text-gray-600 leading-7 mb-8">{car.description || "No description available."}</p>
                        {car.features && (
                            <div>
